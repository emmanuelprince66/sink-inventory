...
const Tier1Form = ({ onComplete }: Tier1FormProps) => {
  const { createIndividualAcctForm, isPending, onSubmitIndividualAcct } =
    useKycHook();

  const [identificationType, setIdentificationType] = useState<"bvn" | "nin">(
    "bvn",
  );

  const handleSubmit = async (data: any) => {
    // Call your existing submit handler
    await onSubmitIndividualAcct(data);
    // If successful, mark tier as complete
    onComplete();
  };
  ...
  return (
    <Form {...createIndividualAcctForm}>
      <form
        ...
        onSubmit={createIndividualAcctForm.handleSubmit(handleSubmit)}
      >
        <FormField
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>
                {identificationType === "bvn" ? "BVN" : "NIN"}
              </FormLabel>
              <FormControl>
                <Input
                  placeholder={
