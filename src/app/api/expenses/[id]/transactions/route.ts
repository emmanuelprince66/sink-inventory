...
export async function GET(
  ...
  const apiUrl = new URL(`${BaseUrl}expenses/business/${id}/transactions/`);
  ...
  try {
    ...
    try {
      ...
      console.error("Non-JSON response fetching transactions:", response.status, rawBody.slice(0, 2000));
    }
    ...
    if (!response.ok) {
      return NextResponse.json(
        {
          success: false,
          message: responseData?.message || responseData?.error || "Failed to fetch transactions",
