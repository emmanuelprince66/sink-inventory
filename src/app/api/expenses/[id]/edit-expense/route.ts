...
export async function POST(
    ...
    return NextResponse.json(
      {
        ...
        message: "No Expense identifier provided",
      },
      { status: 400 }
    );
  }
  ...
  try {
    ...
    const apiUrl = `${BaseUrl}expenses/${expenseId}/`;
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        {
          success: false,
          message: responseData.message || "Failed to update Expense ",
          error: responseData.error || "Expense  update failed",
