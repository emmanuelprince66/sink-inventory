...
export async function GET(
  ...
  const apiUrl = new URL(`${BaseUrl}expenses/business/${id}/categories/`);
  ...
  try {
    ...
    try {
      ...
      console.error("Non-JSON response fetching expense categories:", response.status, rawBody.slice(0, 2000));
    }
    ...
    if (!response.ok) {
      return NextResponse.json(
        {
          success: false,
          message: responseData?.message || responseData?.error || "Failed to fetch categories",
