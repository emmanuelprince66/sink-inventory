...
export async function GET(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        {
          error: errorData.message || "Failed to fetch production history data",
        },
    ...
    return NextResponse.json({
      success: true,
      data,
      message: "Production history data fetched successfully",
    });
  } catch (error) {
    console.error("Error fetching production history data:", error);
    return NextResponse.json(
