...
export async function GET(
  ...
  // Build the API URL
  const apiUrl = new URL(`${BaseUrl}analytic/tax/${id}/`);
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: errorData.message || "Failed to fetch tax analytic data" },
    ...
    return NextResponse.json({
      success: true,
      data,
      message: "Tax analytic data fetched successfully",
    });
  } catch (error) {
    console.error("Error fetching Tax analytic data:", error);
    return NextResponse.json(
