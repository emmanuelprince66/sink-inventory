...
export async function GET(
  ...
  // Build the API URL
  const apiUrl = new URL(`${BaseUrl}analytic/customers/${id}/`);
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        {
          error: errorData.message || "Failed to fetch customer analytic data",
        },
    ...
    return NextResponse.json({
      success: true,
      data,
      message: "customer analytic data fetched successfully",
    });
  } catch (error) {
    console.error("Error fetching customer analytic data:", error);
    return NextResponse.json(
