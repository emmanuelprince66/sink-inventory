...
export async function GET(
  ...
  // Build the API URL
  const apiUrl = new URL(`${BaseUrl}analytic/breakdown/${id}/`);
  ...
  try {
      ...
      return NextResponse.json(
        {
          error:
            errorData.message ||
            "Failed to fetch  analytic bank breakdown data",
        },
    ...
    return NextResponse.json({
      success: true,
      data,
      message: "Bank breakdown analytic data fetched successfully",
    });
  } catch (error) {
