// app/api/analytics/[id]/report/route.ts
...
export async function GET(
  ...
  // Build the API URL
  const apiUrl = new URL(`${BaseUrl}report/${id}/analytics/`);
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        {
          error: errorData.message || "Failed to fetch analytic report data",
        },
    ...
    return NextResponse.json({
      success: true,
      data,
      message: "Report analytic data fetched successfully",
    });
  } catch (error) {
    console.error("Error fetching report analytic data:", error);
    return NextResponse.json(
