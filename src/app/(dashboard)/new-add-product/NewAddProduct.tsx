...
const NewAddProduct = ({
  ...
  const handleEditAll = () => {
    if (productVariations.length > 0) {
      ...
      setEditAllData({
        ...
        discount_threshold: firstVariation.discount_threshold || "", // ✅ ADDED
        expiry_date: firstVariation.expiry_date || "",
      });
    }
  ...
  const applyEditAll = () => {
    ...
    // ✅ CLEAR ERROR IMMEDIATELY AFTER VALIDATION PASSES
    setBulkEditError(null);

    // Apply to only selected product variations
      ...
      if (selectedVariations.includes(variation.id)) {
        Object.entries(editAllData).forEach(([field, value]) => {
          if (value !== "") {
            ...
            form.clearErrors(`product_variations.${index}.${field}` as any);
          }
        });
      }
    });
  ...
  const applyBulkEdit = () => {
    ...
    const isNumericField =
      ...
      bulkEditField === "discount_threshold" ||
                          ...
                          {selectedVariations.length > 0 && (
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent>
                                ...
                                <DropdownMenuItem
                                  onClick={() =>
                                    handleBulkEdit("discount_threshold")
                                  }
                                >
                                  Discount Threshold
                                </DropdownMenuItem>
                                ...
                                <DropdownMenuItem
                                  onClick={() => handleBulkEdit("expiry_date")}
                                >
                                  Expiry Date
                                </DropdownMenuItem>
                                <DropdownMenuItem
                                ...
                                {bulkEditField === "status" ? (
                                  ...
                                  <Popover>
                                    <PopoverTrigger asChild>
                                      ...
                                      >
                                        {bulkEditValue ? (
                                          format(new Date(bulkEditValue), "PPP")
                                        ) : (
        ...
        <div className="grid gap-4 py-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Input
                value={editAllData.discount_threshold}
                onChange={(e) => {
                  setEditAllData({
                    ...editAllData,
                    discount_threshold: e.target.value,
                  });
                  setBulkEditError(null); // ✅ CLEAR ERROR WHEN USER TYPES
                }}
                placeholder="Enter discount threshold"
              />
            </div>
              ...
              <Popover>
                <PopoverTrigger asChild>
                  ...
                  >
                    {editAllData.expiry_date ? (
                      format(new Date(editAllData.expiry_date), "PPP")
                    ) : (
                      <span>Pick expiry date</span>
                    )}
                    <CalendarIcon className="ml-auto h-4 w-4 opacity-50" />
                ...
                <PopoverContent className="w-auto p-0" align="start">
                  <Calendar
                    ...
                    selected={
                      editAllData.expiry_date
                        ? new Date(editAllData.expiry_date)
                        : undefined
                    }
                    onSelect={(date) => {
                      setEditAllData({
                        ...editAllData,
                        expiry_date: date ? date.toISOString() : "",
                      });
                      setBulkEditError(null);
                    }}
                    disabled={(date) =>
